...
import { getMode, parseAnswers } from './policy.js';
...
export async function runChat({ env, messages, emit, abort, mode = 'safety' }) {
  const { questions, name } = getMode(mode);
    ...
    const monitor = createMonitor({
      ...
      evaluate: async (response, complete) => {
        try {
          ...
          return { probabilities: parseAnswers(result, mode), mode, model: result.model, usage: result.usage };
