...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/config') return json(200, { missing, model: env.LLM_MODEL || 'Model not configured', evaluator: env.TYPESAFE_MODEL || 'jev-latest', dimensions });
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        void runChat({ env, messages, emit, abort }).catch(() => {
